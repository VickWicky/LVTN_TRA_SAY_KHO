...
export default function ProductDetail() {
  ...
  const handleAddToCart = () => {
    ...
    toast.success(`Đã thêm ${quantity} hộp ${product.name} (Gói ${selectedVariant.weight}g) vào giỏ hàng!\nTổng tiền: ${(priceToUse * quantity).toLocaleString('vi-VN')}₫`);
  };

  if (isLoading || !product) {
    return <div className="text-center py-20 font-semibold text-primary text-xl">Đang hái trà...</div>;
  }

  // Sử dụng dữ liệu từ Database, nếu không có thì fallback về dữ liệu mẫu
  ...
  const brewList = product.usage_instruction
    ...
    : ['Đun nước sôi đến 75-85°C', 'Cho lá trà 2-3g vào tách', 'Ngâm 3-5 phút rồi thưởng thức'];

  return (
    <div className="container mx-auto px-4 max-w-7xl py-12">
      
      {/* Nút Back về trang sản phẩm cho thân thiện */}
      <div className="mb-6">
        <Link to="/products" className="text-light hover:text-primary transition flex items-center gap-2 font-semibold">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-start">
        ...
        {/* CỘT TRÁI: Ảnh sản phẩm (Sticky để khi cuộn chuột ảnh trượt theo) */}
        ...
        <div>
          ...
          {/* HIỂN THỊ GIÁ (Lấy từ biến thể đang chọn) */}
          <div className="flex items-end gap-4 mb-6">
            <p className="text-4xl font-bold text-primary">
              {selectedVariant ? Number(selectedVariant.sale_price || selectedVariant.price).toLocaleString('vi-VN') : 0}₫
            </p>
            ...
            {selectedVariant && selectedVariant.sale_price && (
              <span className="bg-red-100 text-red-600 px-2 py-1 rounded text-sm font-bold mb-1">
                Tiết kiệm {Number(selectedVariant.price - selectedVariant.sale_price).toLocaleString('vi-VN')}₫
              </span>
            )}
          </div>
          ...
          {/* Tùy chọn Số lượng & Thêm giỏ hàng (Giao diện cũ của bạn) */}
          <div className="flex flex-col sm:flex-row gap-4 mb-10 items-end">
            <div className="flex flex-col w-full sm:w-auto">
              <label className="text-sm font-semibold mb-2 text-dark">Số Lượng:</label>
            ...
            >
              <i className={`${isInWishlist(product.id) ? 'fas' : 'far'} fa-heart text-xl`}></i>
            </button>
          </div>

          {/* Box Chi tiết (Giao diện cũ) */}
          <div className="bg-bglight p-6 rounded-xl mb-6">
            <h3 className="text-lg font-bold text-dark mb-4">Chi Tiết Sản Phẩm</h3>
          ...
          {/* Box Hướng dẫn pha (Giao diện cũ) */}
          <div className="bg-bglight p-6 rounded-xl">
            <h3 className="text-lg font-bold text-dark mb-4">Hướng Dẫn Pha Trà</h3>
