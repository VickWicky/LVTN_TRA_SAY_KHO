  ...
  return (
    <div>
              ...
              <div className="container mx-auto px-4 max-w-7xl">
                <div className="flex justify-between items-end mb-10 border-b border-red-200 pb-4">
                  <div>
                    ...
                    <p className="text-gray-600 mt-2">Đừng bỏ lỡ các sản phẩm đang được giảm giá cực sâu</p>
                  </div>
                  <Link to="/products" className="text-red-500 font-semibold hover:text-red-700 transition">
                    Xem tất cả &rarr;
                  </Link>
                </div>
                
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                  {saleProducts.map((product) => renderProductCard(product, true))}
                </div>
              </div>
            </section>
          )}

          {/* KHU VỰC 2: TOP SẢN PHẨM NỔI BẬT (NGẪU NHIÊN) */}
          {topProducts.length > 0 && (
            <section className="py-16 bg-white">
              <div className="container mx-auto px-4 max-w-7xl">
                <div className="text-center mb-12">
                  <h2 className="text-3xl font-bold text-dark mb-4">Gợi Ý Cho Bạn</h2>
                  <div className="h-1 w-24 bg-primary mx-auto rounded"></div>
                  <p className="text-light mt-4">Những hương vị trà sấy khô tự nhiên được yêu thích nhất</p>
                </div>
      ...
      <section className="bg-bglight py-20 mt-10">
        <div className="container mx-auto px-4 max-w-7xl">
          <h2 className="text-3xl font-bold text-center text-dark mb-12">Tại Sao Chọn CK Tea?</h2>
          
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8 text-center">
            <div className="bg-white p-8 rounded-xl shadow-sm hover:shadow-md transition transform hover:-translate-y-2">
              <div className="text-4xl text-primary mb-4"><i className="fas fa-leaf"></i></div>
              <h3 className="font-bold text-lg mb-2">100% Tự Nhiên</h3>
              <p className="text-light text-sm">Không chứa hóa chất, tất cả đều từ thiên nhiên</p>
            </div>
            
            <div className="bg-white p-8 rounded-xl shadow-sm hover:shadow-md transition transform hover:-translate-y-2">
              <div className="text-4xl text-primary mb-4"><i className="fas fa-check-circle"></i></div>
              <h3 className="font-bold text-lg mb-2">Chất Lượng Đảm Bảo</h3>
              <p className="text-light text-sm">Kiểm tra kỹ lưỡng qua từng khâu sản xuất</p>
            </div>

            <div className="bg-white p-8 rounded-xl shadow-sm hover:shadow-md transition transform hover:-translate-y-2">
              <div className="text-4xl text-primary mb-4"><i className="fas fa-truck"></i></div>
              <h3 className="font-bold text-lg mb-2">Giao Hàng Nhanh</h3>
              <p className="text-light text-sm">Đóng gói cẩn thận và giao tận tay nhanh chóng</p>
            </div>

            <div className="bg-white p-8 rounded-xl shadow-sm hover:shadow-md transition transform hover:-translate-y-2">
              <div className="text-4xl text-primary mb-4"><i className="fas fa-heart"></i></div>
              <h3 className="font-bold text-lg mb-2">Hỗ Trợ Tận Tâm</h3>
              <p className="text-light text-sm">Luôn sẵn sàng giải đáp mọi thắc mắc của bạn</p>
            </div>
          </div>
        </div>
      </section>
