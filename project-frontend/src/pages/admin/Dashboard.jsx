...
export default function Dashboard() {
  ...
  const handleExportExcel = async () => {
    try {
      setIsExporting(true);
      await exportDashboardExcel({ timeRange, startDate, endDate, isSales });
  ...
  const loadMockData = () => {
    ...
    setStats({
      ...
      recent_orders: [
        ...
        {
          id: 4,
          order_code: "ORD-12348",
          shipping_name: "Phạm Thị D",
        ...
        {
          id: 5,
          order_code: "ORD-12349",
          shipping_name: "Hoàng Văn E",
      ...
      top_products: [
        {
          ...
          total_sold: 150 + Math.floor(Math.random() * 20),
        },
        {
          name: "Trà Xanh Thái Nguyên",
          total_sold: 120 + Math.floor(Math.random() * 15),
        },
        {
          name: "Trà Sen Cổ Thụ",
          total_sold: 95 + Math.floor(Math.random() * 10),
        },
  ...
  const renderTrend = (value) => {
    ...
    return (
      <div
        className={`flex items-center gap-1 text-xs font-semibold px-2 py-1 rounded-full ${isPositive ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
      >
        <i
          className={`fas ${isPositive ? "fa-arrow-up" : "fa-arrow-down"}`}
        ></i>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1 text-sm">
            Theo dõi chi tiết tình hình kinh doanh của bạn
          </p>
        </div>
...
hover:bg-green-100 transition-colors text-sm font-medium text-green-700 ${isExporting ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}`}
          ...
          >
            <i
              className={`fas fa-sync-alt text-primary ${isLoading ? "animate-spin" : ""}`}
            ></i>
          </button>
        </div>
      </div>
      ...
      {/* STATS CARDS */}
      <div
        className={`grid grid-cols-1 md:grid-cols-2 ${isSales ? "lg:grid-cols-3" : "lg:grid-cols-4"} gap-5`}
      >
        {!isSales && (
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 lg:col-span-8">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-lg font-bold text-gray-800">
              Biến động {isSales ? "Đơn hàng" : "Doanh thu & Đơn hàng"}
            </h3>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 lg:col-span-4 flex flex-col">
          ...
          <div className="flex-1 w-full flex items-center justify-center min-h-[250px]">
            {formatStatusData.length > 0 ? (
              <OrderStatusPieChart data={formatStatusData} />
            ) : (
              <div className="text-gray-400 text-sm border-2 border-dashed border-gray-100 rounded-xl p-8">
                Chưa có dữ liệu
              </div>
            )}
          </div>
            ...
            <div className="space-y-6 flex-1 mt-2">
              {stats.top_products.map((product, index) => {
                const maxSold = Math.max(
                  ...stats.top_products.map((p) => Number(p.total_sold)),
                );
                ...
                return (
                  <div key={index} className="flex items-center gap-4 group">
                    <div
                      className={`w-12 h-12 rounded-xl flex items-center justify-center font-bold text-base shrink-0 transition-all shadow-sm ${
                        index === 0
                          ? "bg-gradient-to-br from-yellow-100 to-yellow-200 text-yellow-700 border border-yellow-300/50"
                          : index === 1
                            ? "bg-gradient-to-br from-gray-100 to-gray-200 text-gray-600 border border-gray-300/50"
                            : index === 2
                              ? "bg-gradient-to-br from-orange-100 to-orange-200 text-orange-700 border border-orange-300/50"
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between mb-1.5">
                        <p className="text-sm font-semibold text-gray-800 truncate pr-2 group-hover:text-primary transition-colors">
                          {product.name}
                        </p>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
        <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-transparent">
          ...
          <Link
            to="/admin/orders"
            className="text-sm text-primary hover:text-primary-dark font-medium px-3 py-1.5 rounded-lg hover:bg-primary/5 transition-colors cursor-pointer"
          >
            Xem tất cả
          </Link>
        </div>
            ...
            <tbody className="divide-y divide-gray-100 bg-white">
              {(stats.recent_orders || []).length === 0 ? (
                <tr>
                  <td
                    colSpan="6"
                    className="px-6 py-12 text-center text-gray-500 border-2 border-dashed border-gray-100 rounded-xl m-4"
                  >
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        {order.payment_method === "vnpay" ? (
                          <span className="w-6 h-4 bg-blue-100 rounded text-[8px] font-bold text-blue-700 flex items-center justify-center">
                            VN
                          </span>
                        ) : order.payment_method === "momo" ? (
                          <span className="w-6 h-4 bg-pink-100 rounded text-[8px] font-bold text-pink-700 flex items-center justify-center">
                            MM
                          </span>
                        ) : (
                          <span className="w-6 h-4 bg-green-100 rounded text-[8px] font-bold text-green-700 flex items-center justify-center">
