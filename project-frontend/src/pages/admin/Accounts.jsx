...
export default function Accounts() {
  ...
  const fetchData = async (page = currentPage, search = debouncedSearch) => {
    try {
      ...
      const res = await fetch(
        `${API_URL}/api/admin/accounts?page=${page}&per_page=${perPage}&search=${encodeURIComponent(search)}`,
        {
          headers: {
  ...
  const openEditDrawer = (user) => {
    ...
    const currentRole =
      user.roles && user.roles.length > 0 ? user.roles[0].name : "customer";
    setFormData({
      name: user.name || "",
      email: user.email || "",
      phone: user.phone || "",
  ...
  const executeToggleStatus = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(
        `${API_URL}/api/admin/accounts/${user.id}/status`,
        {
          method: "PUT",
          headers: {
            Accept: "application/json",
  ...
  const renderRoleBadge = (roleName) => {
    switch (roleName) {
      case "admin":
        return (
          <span className="bg-red-50 border border-red-200 text-red-700 px-3 py-1 rounded-full text-xs font-bold shadow-sm">
            Admin
          </span>
        );
      case "staff":
      ...
      case "sales":
        return (
          <span className="bg-amber-50 border border-amber-200 text-amber-700 px-3 py-1 rounded-full text-xs font-bold shadow-sm">
            Bán hàng (Sales)
          </span>
        );
      case "customer":
        return (
          <span className="bg-gray-100 border border-gray-200 text-gray-700 px-3 py-1 rounded-full text-xs font-bold shadow-sm">
            Khách hàng
          </span>
        );
      default:
        const roleObj = rolesList.find((r) => r.name === roleName);
        const displayName = roleObj
          ? roleObj.name.toUpperCase()
          : roleName
            ? roleName.toUpperCase()
            : "Khách hàng";
        return (
          <span className="bg-indigo-50 border border-indigo-200 text-indigo-700 px-3 py-1 rounded-full text-xs font-bold shadow-sm">
            {displayName}
          </span>
        );
    }
              ...
              {users.length === 0 ? (
                ...
                users.map((user) => {
                  ...
                  return (
                    <tr
                      key={user.id}
                      className={`hover:bg-blue-50/30 transition-colors group ${isCurrentUser ? "bg-blue-50/10" : ""}`}
                    >
                      <td className="p-4 text-center font-medium text-gray-500 text-sm">
                        #{user.id}
                      </td>
                      <td className="p-4">
                          ...
                          {isCurrentUser ? (
                            ...
                            <>
                              ...
                              {openMenuId === user.id && (
                                <div
                                  ref={menuRef}
                                  className="absolute right-12 top-8 w-44 bg-white rounded-xl shadow-xl border border-gray-100 overflow-hidden z-20 animate-fade-in text-left"
                                >
                                  <button
                                  ...
                                  <div className="border-t border-gray-100">
                                    ...
                                    >
                                      <i
                                        className={`fas ${user.is_active ? "fa-lock" : "fa-unlock"} w-4`}
                                      ></i>
                                      {user.is_active
                                        ? "Khóa tài khoản"
                                        : "Mở khóa tài khoản"}
                ...
                >
                  {drawerMode === "edit" && (
                    <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 text-sm text-blue-800 flex items-start gap-3">
                      ...
                      <div>
                        <p>
                          Đang chỉnh sửa: <strong>{selectedUser?.email}</strong>
                        </p>
                  ...
                  <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm space-y-4">
                    ...
                    <div>
                      ...
                      >
                        {rolesList.map((r) => {
                          if (drawerMode === "create" && r.name === "customer")
                            return null;
                          return (
                  ...
                  <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm space-y-4">
                    ...
                    <div>
                      <p className="text-xs text-gray-500 mb-2">
                        {drawerMode === "create"
                          ? "Tạo mật khẩu cho tài khoản nội bộ (ít nhất 6 ký tự)."
                          : "Chỉ nhập khi bạn muốn thay đổi mật khẩu của người này."}
                      </p>
                      <input
                        type="password"
                        name="password"
                ...
                <div className="space-y-6 animate-fade-in">
                  <div className="bg-amber-50 border border-amber-200 rounded-xl p-5">
                    ...
                    <p className="text-sm text-gray-700 mb-4 leading-relaxed">
                      {drawerMode === "create" ? (
                        <>
                          Bạn đang tạo tài khoản nội bộ mới cho{" "}
                    ...
                    <div className="bg-white rounded-lg p-4 space-y-3 border border-amber-100">
                      <div className="flex justify-between items-center pb-2 border-b border-gray-50">
                        <span className="text-gray-500 text-sm">
                          Vai trò {drawerMode === "create" ? "được cấp" : "mới"}
                        </span>
                      ...
                      {formData.password && (
                        <div className="flex justify-between items-center text-red-600 font-bold">
                          <span className="text-sm">
                            <i className="fas fa-lock mr-1"></i> Mật khẩu
                          </span>
              ...
              {step === 1 ? (
                <>
                  ...
                  <button
                    form="accountForm"
                    type="submit"
                    className="px-8 py-2.5 bg-primary text-white rounded-xl font-bold hover:bg-primary-dark shadow-md shadow-primary/30 flex items-center gap-2 transition-all active:scale-95"
                  >
                ...
                <>
                  ...
                  <button
                    ...
                    className="px-8 py-2.5 bg-red-600 text-white rounded-xl font-bold hover:bg-red-700 shadow-md shadow-red-500/30 flex items-center gap-2 transition-all active:scale-95"
                  >
                    <i className="fas fa-check"></i>{" "}
                    {drawerMode === "create" ? "Tạo Tài Khoản" : "Lưu Thay Đổi"}
                  </button>
                </>
              )}
            </div>
