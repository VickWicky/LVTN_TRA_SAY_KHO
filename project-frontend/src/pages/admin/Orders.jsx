...
export default function Orders() {
      ...
      if (res.ok) {
        ...
        if (Array.isArray(data)) {
          filteredData =
            status === "all"
              ? data
              : data.filter((o) => o.order_status === status);
          if (search) {
  ...
  const handleUpdateShipping = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(
        `${API_URL}/api/admin/orders/${shippingModal.orderId}/shipping`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case "completed":
        return (
          <span className="bg-emerald-50 text-emerald-600 border border-emerald-200 px-3 py-1 rounded-full text-xs font-bold shadow-sm">
            Hoàn thành
          </span>
        );
      case "cancelled":
      ...
      case "processing":
        return (
          <span className="bg-blue-50 text-blue-600 border border-blue-200 px-3 py-1 rounded-full text-xs font-bold shadow-sm">
            Đang chuẩn bị
          </span>
        );
      case "shipping":
        return (
          <span className="bg-purple-50 text-purple-600 border border-purple-200 px-3 py-1 rounded-full text-xs font-bold shadow-sm">
            Đã Bàn Giao Vận Tải
          </span>
        );
      default:
        return (
          <span className="bg-amber-50 text-amber-600 border border-amber-200 px-3 py-1 rounded-full text-xs font-bold shadow-sm">
            Chờ xác nhận
          </span>
        );
    }
  ...
  const getPaymentBadge = (status) => {
    switch (status) {
      ...
      case "refunded":
        return (
          <span className="bg-purple-100 text-purple-700 px-2.5 py-1 rounded text-xs font-bold">
            Đã hoàn tiền
          </span>
        );
      case "failed":
      ...
      default:
        return (
          <span className="bg-yellow-100 text-yellow-700 px-2.5 py-1 rounded text-xs font-bold">
            Chờ thanh toán
          </span>
        );
    }
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">
              Theo dõi, cập nhật và xử lý đơn hàng
            </p>
          </div>
                  ...
                  >
                    ...
                    <td className="p-4 text-center relative">
                      ...
                      {openMenuId === order.id && (
                        <div
                          ref={menuRef}
                          className="absolute right-8 top-10 w-48 bg-white rounded-xl shadow-xl border border-gray-100 overflow-hidden z-20 animate-fade-in text-left"
                        >
                          <button
                            ...
                            order.order_status !== "cancelled" && (
                              <div className="border-t border-gray-100">
                                ...
                                {order.order_status !== "pending" && (
                                  <button
                                    onClick={() =>
                                      onStatusSelectChange(order.id, "pending")
                                    }
                                    className="w-full px-4 py-2 text-sm text-gray-600 hover:bg-gray-200 text-left flex items-center gap-2"
                                  >
                                ...
                                {order.order_status !== "shipping" && (
                                  <button
                                    onClick={() =>
                                      onStatusSelectChange(order.id, "shipping")
                                    }
                                    className="w-full px-4 py-2 text-sm text-gray-600 hover:bg-gray-200 text-left flex items-center gap-2"
                                  >
            ...
            <div className="px-6 py-5 border-b border-gray-100 flex justify-between items-center bg-white z-10">
              <div>
                <h3 className="text-xl font-bold text-gray-900 flex items-center gap-3">
                  Chi tiết đơn hàng
                  <span className="bg-blue-50 text-primary px-3 py-1 rounded-md text-sm">
                    #{selectedOrder.order_code}
                  </span>
                </h3>
                <p className="text-sm text-gray-500 mt-1">
                  <i className="far fa-clock mr-1"></i> Đặt lúc:{" "}
            ...
            <div className="flex-1 overflow-y-auto p-6 bg-gray-50/50 custom-scrollbar">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mb-6">
                <div className="px-5 py-4 border-b border-gray-100 bg-gray-50/50">
                  <h4 className="font-bold text-gray-800">
                    <i className="fas fa-box-open text-primary mr-2"></i>Sản
                    phẩm đã đặt
                  </h4>
                </div>
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-500 mb-2">
                          Phân loại: {item.variant?.weight}g (SKU:{" "}
                          {item.variant?.sku})
                        </p>
                        <div className="flex justify-between items-center">
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div className="pt-4 border-t border-gray-200 mt-2 flex justify-between items-center">
                    ...
                    <span className="text-2xl font-bold text-primary">
                      {Number(selectedOrder.final_amount).toLocaleString(
                        "vi-VN",
                      )}
                      ₫
                    </span>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 relative z-10 animate-fade-in text-center transform scale-100">
            ...
            {confirmModal.newStatus === "cancelled" && (
              <div className="bg-red-50 text-red-600 p-3 rounded-lg text-xs font-medium mb-6 text-left border border-red-100">
                ...
                <strong>Lưu ý:</strong> Nếu đơn hàng đã thanh toán qua VNPAY, hệ
                thống sẽ tự động gửi lệnh hoàn tiền cho khách hàng.
              </div>
            )}
            {confirmModal.newStatus !== "cancelled" && (
