...
export default function AdminContacts() {
  ...
  const fetchContacts = async (page = currentPage, search = debouncedSearch) => {
    setIsLoading(true);
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${API_URL}/api/admin/contacts?page=${page}&search=${encodeURIComponent(search)}`, {
        headers: {
  ...
  const handleOpenDetails = (contact) => {
    ...
    setOpenMenuId(null);
    // Tự động đánh dấu "Đã xem" nếu trạng thái đang là "new"
      ...
      case 'new': 
        return (
          <span className="bg-blue-50 text-blue-600 border border-blue-200 px-3 py-1 rounded-full text-[11px] font-bold shadow-sm flex items-center gap-1.5 whitespace-nowrap">
            <span className="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse"></span> Mới nhận
          </span>
        );
      case 'read': 
        return (
    ...
    <div className="space-y-6 pb-10">
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h2 className="text-2xl font-bold text-gray-900 tracking-tight">Quản Lý Liên Hệ</h2>
            <p className="text-gray-500 text-sm mt-1">Hỗ trợ và phản hồi từ khách hàng</p>
          </div>
                ...
                <tr>
                  <td colSpan="5" className="p-16 text-center">
                    <div className="flex flex-col items-center justify-center text-gray-400">
                      ...
                      <p className="text-base font-medium">Chưa có liên hệ nào</p>
                    </div>
                  </td>
                </tr>
              ) : contacts.map(contact => (
                <tr key={contact.id} className={`hover:bg-blue-50/30 transition-colors group ${contact.status === 'new' ? 'bg-blue-50/10' : ''}`}>
                  <td className="p-4">
                    <div className="font-bold text-gray-900 text-sm mb-1">{contact.name}</div>
                    <div className="text-xs text-gray-500 font-medium"><i className="fas fa-envelope text-gray-400 mr-1.5"></i>{contact.email}</div>
                    {contact.phone && <div className="text-xs text-gray-500 mt-1"><i className="fas fa-phone-alt text-gray-400 mr-1.5"></i>{contact.phone}</div>}
                  </td>
                  <td className="p-4">
                    <div className={`text-sm mb-1 line-clamp-1 ${contact.status === 'new' ? 'font-bold text-gray-900' : 'font-semibold text-gray-700'}`}>{contact.subject}</div>
                  ...
                  <td className="p-4 text-center relative">
                    ...
                    {openMenuId === contact.id && (
                      <div ref={menuRef} className="absolute right-12 top-10 w-44 bg-white rounded-xl shadow-xl border border-gray-100 overflow-hidden z-20 animate-fade-in text-left">
                        <button 
                        ...
                        >
                          <i className="fas fa-expand-arrows-alt w-4"></i> Xem chi tiết
                        </button>
                        ...
                        <div className="border-t border-gray-100">
                          {contact.status !== 'resolved' && (
                            ...
                            >
                              <i className="fas fa-check-double w-4"></i> Đánh dấu đã xử lý
                            </button>
                          )}
                          <button 
        ...
        <div className="fixed inset-0 z-[60] flex justify-end">
          ...
          <div className="relative w-full max-w-lg bg-white h-full shadow-2xl flex flex-col animate-slide-in-right">
            ...
            <div className="flex-1 overflow-y-auto p-6 bg-gray-50/50 custom-scrollbar space-y-6">
              ...
              <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm flex flex-col gap-3">
                <div className="flex justify-between items-center">
                  <span className="text-sm font-bold text-gray-700">Trạng thái hiện tại:</span>
              ...
              <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm space-y-4">
                <h4 className="font-bold text-gray-800 border-b border-gray-100 pb-2">Thông tin người gửi</h4>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex flex-col gap-1 mt-1 text-sm text-gray-600">
                      <div className="flex items-center gap-2"><i className="fas fa-envelope w-4 text-center text-gray-400"></i> {selectedContact.email}</div>
                      <div className="flex items-center gap-2"><i className="fas fa-phone-alt w-4 text-center text-gray-400"></i> {selectedContact.phone || <span className="italic text-gray-400">Không cung cấp</span>}</div>
                      <div className="flex items-center gap-2"><i className="far fa-clock w-4 text-center text-gray-400"></i> {new Date(selectedContact.created_at).toLocaleString('vi-VN')}</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm space-y-3">
                <div className="border-b border-gray-100 pb-3">
                  <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Chủ đề</h4>
                  <p className="font-bold text-gray-900 text-lg">{selectedContact.subject}</p>
                </div>
                <div>
                  <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Nội dung chi tiết</h4>
            ...
            <div className="px-6 py-4 border-t border-gray-100 bg-white flex justify-end gap-3 z-10 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
              <button onClick={() => setIsDrawerOpen(false)} className="px-6 py-2.5 bg-gray-100 text-gray-700 rounded-xl font-bold hover:bg-gray-200 transition-colors w-full sm:w-auto">
                Đóng lại
              </button>
            </div>
          </div>
