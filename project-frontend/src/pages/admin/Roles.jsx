...
export default function Roles() {
  ...
  const CORE_ROLES = ['admin', 'customer', 'staff', 'sales'];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success(drawerMode === 'create' ? 'Tạo vai trò thành công!' : 'Cập nhật vai trò thành công!');
        closeDrawer();
        fetchRolesAndPermissions();
      } else {
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 tracking-tight">Quản Lý Vai Trò</h2>
          <p className="text-gray-500 text-sm mt-1">Cấu hình phân quyền chi tiết cho nhân viên</p>
        </div>
        <button 
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100 text-gray-600 text-sm">
                  ...
                  <th className="py-4 px-6 font-semibold text-center">Người Dùng</th>
                  <th className="py-4 px-6 font-semibold">Các Quyền (Permissions)</th>
                  ...
                  <tr key={role.id} className="hover:bg-gray-50 transition-colors">
                    <td className="py-4 px-6">
                      ...
                      {CORE_ROLES.includes(role.name) && (
                        <span className="ml-2 text-[10px] bg-red-100 text-red-600 px-2 py-0.5 rounded-full uppercase font-bold tracking-wider">Mặc định</span>
                      )}
                    </td>
                    ...
                    <td className="py-4 px-6">
                      <div className="flex flex-wrap gap-1 max-w-md">
                        {role.name === 'admin' ? (
                          <span className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded border border-green-200">Full Quyền</span>
                        ) : role.permissions.length > 0 ? (
                          role.permissions.slice(0, 4).map(p => (
                            <span key={p.id} className="text-xs bg-blue-50 text-blue-700 px-2 py-1 rounded border border-blue-100">
                              {p.name}
                            </span>
                          ))
                        ) : (
                        ...
                        {role.permissions.length > 4 && (
                          <span className="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded">+{role.permissions.length - 4}</span>
                        )}
                      </div>
                    </td>
                    <td className="py-4 px-6">
      ...
      {isDrawerOpen && (
        <div className="fixed inset-0 z-50 overflow-hidden">
          <div className="absolute inset-0 bg-gray-900/50 backdrop-blur-sm transition-opacity" onClick={closeDrawer} />
            ...
            <div className="w-full h-full bg-white shadow-2xl flex flex-col transform transition-transform">
              <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50">
                <h3 className="text-lg font-bold text-gray-900">
                  {drawerMode === 'create' ? 'Thêm Vai Trò Mới' : 'Cập Nhật Phân Quyền'}
                </h3>
                <button onClick={closeDrawer} className="text-gray-400 hover:text-red-500 transition-colors p-2">
              ...
              <div className="flex-1 overflow-y-auto p-6">
                <form id="roleForm" onSubmit={handleSubmit} className="space-y-6">
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-2">Tên Vai Trò (Ví dụ: Kế toán)</label>
                    <input 
                      type="text" 
                      required
                      readOnly={drawerMode === 'edit' && ['admin', 'customer'].includes(formData.name)}
                      value={formData.name}
                      ...
                      className={`w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-colors ${
                        (drawerMode === 'edit' && ['admin', 'customer'].includes(formData.name)) ? 'opacity-60 cursor-not-allowed' : ''
                      }`}
                    />
                    {(drawerMode === 'edit' && ['admin', 'customer'].includes(formData.name)) && (
                      <p className="text-xs text-amber-600 mt-1"><i className="fas fa-info-circle"></i> Tên vai trò mặc định không thể thay đổi.</p>
                    )}
                  </div>

                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-3">Gán Quyền Hạn (Permissions)</label>
                    {formData.name === 'admin' ? (
                      <div className="p-4 bg-green-50 border border-green-200 rounded-lg text-green-700 text-sm">
                        <i className="fas fa-shield-alt mr-2"></i> Role <b>admin</b> mặc định có toàn quyền.
                      </div>
                    ) : (
                      <div className="space-y-3 bg-gray-50 p-4 rounded-xl border border-gray-100 h-96 overflow-y-auto">
                        {permissions.map(p => (
                          <label key={p.id} className="flex items-start gap-3 p-2 hover:bg-white rounded-lg cursor-pointer transition-colors border border-transparent hover:border-gray-200">
