...
export default function Contact() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Kiểm tra nhanh điều kiện dữ liệu (Validation) trước khi đẩy qua API
    ...
    try {
      ...
      if (response.ok) {
        toast.success('Cảm ơn bạn đã gửi thông tin. Chúng tôi sẽ phản hồi trong thời gian sớm nhất!');
    ...
    <div>
      ...
      <section className="bg-gradient-to-br from-primary to-primary-light text-white py-16 text-center">
        <div className="container mx-auto px-4 max-w-7xl">
          ...
          <p className="text-lg opacity-90">Chúng tôi luôn sẵn sàng lắng nghe ý kiến của bạn</p>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4 max-w-7xl">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div className="bg-bglight p-8 rounded-xl shadow-sm">
              <h2 className="text-2xl font-bold text-dark mb-6">Gửi Thông Tin Phản Hồi</h2>
            ...
            {/* THÔNG TIN LIÊN HỆ BÊN CẠNH */}
            <div className="flex flex-col justify-center space-y-8 pl-0 lg:pl-6">
              <div>
                <h2 className="text-2xl font-bold text-dark mb-6">Thông Tin Chi Tiết</h2>
              </div>
              ...
              {settings.branches && settings.branches.length > 0 ? (
                settings.branches.map((branch, index) => (
                  <div key={index} className="flex gap-4 items-start mb-6">
                    <div className="text-2xl text-primary p-3 bg-bglight rounded-lg">📍</div>
                    <div>
                      <h3 className="font-bold text-lg text-dark mb-1">{branch.name}</h3>
                ...
                <div className="flex gap-4 items-start mb-6">
                  <div className="text-2xl text-primary p-3 bg-bglight rounded-lg">📍</div>
                  <div>
                    <h3 className="font-bold text-lg text-dark mb-1">Trụ Sở Chính</h3>
                    <p className="text-light text-sm">123 Đường Trà Sấy, Quận Ba Đình, Hà Nội</p>
                  </div>
                </div>
              )}
              ...
              <div className="flex gap-4 items-start mb-6">
                <div className="text-2xl text-primary p-3 bg-bglight rounded-lg">📞</div>
                <div>
                  <h3 className="font-bold text-lg text-dark mb-1">Điện Thoại</h3>
                  <p className="text-light text-sm">{settings.contact_phone || '0123 456 789'}</p>
                  <p className="text-light text-xs">Hotline phục vụ: 8h - 22h tất cả các ngày</p>
                </div>
              </div>

              <div className="flex gap-4 items-start mb-6">
                <div className="text-2xl text-primary p-3 bg-bglight rounded-lg">✉️</div>
                <div>
                  <h3 className="font-bold text-lg text-dark mb-1">Email</h3>
                  <p className="text-light text-sm">{settings.contact_email || 'hello@cktea.vn'}</p>
                </div>
              </div>
              ...
              <div className="flex gap-4 items-start">
                <div className="text-2xl text-primary p-3 bg-bglight rounded-lg">🕒</div>
                <div>
                  <h3 className="font-bold text-lg text-dark mb-1">Giờ Mở Cửa</h3>
                  <p className="text-light text-sm">Thứ 2 - Thứ 6: 8:00 - 18:00</p>
                  <p className="text-light text-sm">Thứ 7 - Chủ Nhật: 9:00 - 17:00</p>
                </div>
              </div>
            </div>
