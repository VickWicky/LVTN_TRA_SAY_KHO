      ...
      <div className="max-w-lg w-full bg-white p-8 rounded-xl shadow-lg">
        <div className="text-center mb-8">
          ...
          <p className="text-light text-sm">
            {step === 1 ? 'Trở thành thành viên của CK Tea ngay hôm nay' : 'Xác thực địa chỉ Email của bạn'}
          </p>
        </div>
        ...
        {step === 1 ? (
          <form onSubmit={handleRegisterSubmit} className="space-y-5">
            ...
            <div>
              <label className="block text-sm font-semibold text-dark mb-2">Mật Khẩu</label>
              <input 
                type="password" 
                required
                minLength="6"
            ...
            <div>
              <label className="block text-sm font-semibold text-dark mb-2">Xác Nhận Mật Khẩu</label>
              <input 
                type="password" 
                required
                className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:border-primary transition"
          ...
          <form onSubmit={handleOtpSubmit} className="space-y-5">
            <div className="bg-blue-50 border border-blue-200 text-blue-700 px-4 py-3 rounded-lg text-sm mb-4">
              Chúng tôi đã gửi một mã OTP gồm 6 chữ số đến email <strong>{formData.email}</strong>. Vui lòng kiểm tra hộp thư (bao gồm cả mục Spam).
            </div>
            <div>
              <label className="block text-sm font-semibold text-dark mb-2 text-center">Nhập mã OTP</label>
              <input 
                type="text" 
                required
                maxLength="6"
