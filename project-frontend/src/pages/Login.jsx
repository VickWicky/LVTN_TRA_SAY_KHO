...
export default function Login() {
  ...
  const redirectByRole = (roles, userName, permissions = []) => {
    ...
    if (roles.includes("admin") || permissions.includes("view-dashboard")) {
    ...
    } else if (roles.some((r) => r !== "user" && r !== "customer")) {
    ...
    onSuccess: async (tokenResponse) => {
      try {
        ...
        if (res.ok) {
          ...
          redirectByRole(roles, data.user.name, data.permissions || []);
        } else {
      ...
      <div className="max-w-md w-full bg-white p-8 rounded-xl shadow-lg">
        <div className="text-center mb-8">
          ...
          <p className="text-light text-sm">
            Chào mừng bạn quay trở lại với CK Tea
          </p>
        </div>
