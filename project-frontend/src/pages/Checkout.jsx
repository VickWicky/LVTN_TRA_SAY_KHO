...
export default function Checkout() {
  ...
  useEffect(() => {
    ...
    if (savedFormData) {
      ...
      return; // Bỏ qua fetch thông tin user để giữ lại thông tin người dùng đã nhập trước khi login
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (formData.payment_method === 'vnpay') {
          ...
          try {
            ...
            if (vnpayRes.ok && vnpayData.vnpay_url) {
              ...
              return; // Dừng thực thi, trình duyệt sẽ chuyển hướng
            } else {
              toast.error('Lỗi khi tạo giao dịch VNPAY. Đơn hàng đã chuyển về dạng thanh toán sau.');
            }
          } catch (e) {
            console.error("Lỗi VNPAY:", e);
            toast.error('Không thể kết nối đến cổng thanh toán. Đơn hàng đã chuyển về dạng thanh toán sau.');
          }
        }
        ...
        if (token) {
          ...
          toast.success('Đặt hàng thành công! Nhân viên sẽ liên hệ với bạn sớm nhất có thể.');
          navigate('/'); 
        }
      } else {
  ...
  if (cartItems.length === 0) {
    return (
      <div className="container mx-auto px-4 py-16 text-center">
        ...
        <button onClick={() => navigate('/products')} className="bg-primary text-white px-6 py-2 rounded-lg hover:bg-primary-dark transition">
          Tiếp tục mua sắm
        </button>
      </div>
    );
  ...
  return (
    <div className="container mx-auto px-4 max-w-6xl py-12 flex-grow animate-fade-in">
      <h1 className="text-3xl font-bold text-dark mb-8 border-b pb-4">Thanh Toán</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
          <h2 className="text-xl font-bold text-dark mb-6">Thông Tin Người Nhận</h2>
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <label className="block text-sm font-semibold text-dark mb-2">Địa Chỉ Nhận Hàng *</label>
              <textarea 
            ...
            <div>
              ...
              <div className="space-y-3">
                <label className={`border rounded-lg p-4 flex items-center justify-between cursor-pointer transition ${formData.payment_method === 'cod' ? 'border-primary bg-primary-light/5' : 'border-gray-300 bg-white hover:bg-gray-50'}`}>
                  <div className="flex items-center gap-3">
                    ...
                    />
                    <span className="font-semibold text-dark">Thanh toán khi nhận hàng (COD)</span>
                  </div>
                  <i className="fas fa-money-bill-wave text-green-600 text-xl"></i>
                </label>

                <label className={`border rounded-lg p-4 flex flex-col justify-between transition ${formData.payment_method === 'vnpay' ? 'border-primary bg-primary-light/5' : 'border-gray-300 bg-white hover:bg-gray-50'} ${!localStorage.getItem('token') ? 'opacity-70 cursor-not-allowed' : 'cursor-pointer'}`}>
                  <div className="flex items-center justify-between w-full">
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <span className="font-semibold text-dark">Thanh toán trực tuyến bằng VNPAY</span>
                    </div>
            ...
            <div className="pt-6 border-t mt-8">
              <button 
                ...
                className={`w-full py-4 rounded-xl font-bold text-lg transition shadow-md flex items-center justify-center gap-2 ${
                  isLoading ? 'bg-gray-400 text-white cursor-not-allowed' : 'bg-primary text-white hover:bg-primary-dark hover:-translate-y-1 cursor-pointer'
                }`}
              >
                {isLoading ? (
                  <>
        ...
        <div className="bg-gray-50 p-8 rounded-2xl border border-gray-200 h-fit sticky top-24">
          ...
          <div className="space-y-4 mb-6 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
            {cartItems.map((item, index) => {
              const price = item.variant.sale_price > 0 ? item.variant.sale_price : item.variant.price;
              return (
                <div key={index} className="flex gap-4 items-center bg-white p-3 rounded-xl border border-gray-100 shadow-sm">
                  ...
                  <div className="flex-1">
                    <h3 className="font-bold text-dark text-sm line-clamp-1">{item.product.name}</h3>
                    <p className="text-xs text-gray-500 mt-1">Loại: {item.variant.weight}g</p>
                    <div className="flex justify-between items-center mt-2">
                      <span className="text-xs font-semibold text-gray-700">SL: x{item.quantity}</span>
                      <span className="text-primary font-bold text-sm">{(price * item.quantity).toLocaleString()}₫</span>
                    </div>
                  </div>
                </div>
              );
